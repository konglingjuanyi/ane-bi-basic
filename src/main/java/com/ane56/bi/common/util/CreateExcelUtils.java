...
public class CreateExcelUtils {
	...
	// 创建excel文件和样式
	public static HSSFSheet createExcelTitleAndStyle(String worksheetTitle,
			HSSFWorkbook wb, Map<String, String> excelTitleMap, int num) {
		if (num > 0) {
			num--;
		}
		// 创建一个sheet
		...
		if (excelTitleMap != null) {
			...
			for (String key : excelTitleMap.keySet()) {
				if (i == 0) {
					// 第一行第一列
					cell1.setCellValue(new HSSFRichTextString(excelTitleMap
							.get(key)));
	...
	public static void paraseDataToExcelSheet(List<Map<String, Object>> value,
			HSSFSheet sheet, Map<String, String> excelTitleMap) {
		int indexRow = 2; // 行号
		int indexCell = 0; // 列
