...
public class StringUtils {
	...
	/**
	 * 将数字转成金额表示，如11234 为 11,234.00
	 * 
	 * @param num
	 * @return
	 */
	public static String convertNumToMoney(Object num) {
	...
	/**
	* @Title: inSql
	* @Description: 将list 转换为sql 中in操作
	* @param @param dataList
	* @param @return
	* @return String
	* @throws
	 */
	public static String inSql(List<String> dataList){
		if(dataList == null || dataList.isEmpty())
			return null;
		StringBuffer in = new StringBuffer();
		for(String datas : dataList){
			in.append("'").append(datas).append("',");
		}
	...
	public static String formatNum(String number) {
		...
		number = new StringBuilder(number).reverse().toString();     //先将字符串颠倒顺序  
        for(int i=0;i<number.length();i++){  
            if(i*3+3>number.length()){  
            	temp +=number.substring(i*3, number.length());  
                break;  
            }  
            temp+= (number.substring(i*3, i*3+3)+ ",");  
        }  
        if(temp.endsWith(",")){  
	...
	public static String format2Decimal(Object o) {
	...
	public static boolean isImage(String fileName) {
		if("image/jpg".equals(fileName) ||
				"image/jpeg".equals(fileName) ||
				"image/gif".equals(fileName) ||
				"image/png".equals(fileName) ||
				"image/bmp".equals(fileName)){
			return true;		
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static String trimExcessSpace(String str) {
	...
	/**
	 * 去除Mysql中"2015-02-10 15:04:46.0"---->>"2015-02-10 15:04:46"
	 *@param str
	 *@return
	 */
